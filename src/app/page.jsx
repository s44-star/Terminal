...
/* src/app/page.jsx
 * LEXCONV // GTM Intelligence Terminal.
 * Real on-chain TON payment via TonConnect. Plain JavaScript. */
...
import { TONKEEPER_WALLET_ADDRESS, USD_PER_TON } from '../config';

/* ---------------- payment config ---------------- */
const RECIPIENT_ADDRESS = TONKEEPER_WALLET_ADDRESS;

const IS_PLACEHOLDER =
  !RECIPIENT_ADDRESS ||
  RECIPIENT_ADDRESS.includes('PASTE_YOUR') ||
  /^0:0{60,}$/.test(RECIPIENT_ADDRESS);

const tonFor = (usd) => usd / USD_PER_TON;
const nanoFor = (usd) => Math.round(tonFor(usd) * 1e9).toString();
const fmtTon = (ton) => (Number.isInteger(ton) ? ton.toString() : ton.toFixed(2));
...
function scoreFrom(seed, floor, ceil) {
  const span = ceil - floor;
  return floor + (hash(seed) % (span + 1));
}

function buildReport(brand, offer, segment, message) {
...
/* ---------------- allocation engine (strict if/then) ---------------- */
function allocate(budget) {
...
export default function Page() {
  ...
  const confirmPay = async () => {
    ...
    if (IS_PLACEHOLDER) {
      setPayError('Recipient not set. Paste your wallet address in src/config.js.');
      return;
    }
                  ...
                  <div className="space-y-5">
                    <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-4">
                      ...
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {[50, 250, 1000, 5000].map((bb) => (
                          <button
                            key={bb}
                            onClick={() => setBudget(bb)}
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 backdrop-blur-sm sm:items-center">
          <div className="w-full max-w-sm rounded-t-2xl border border-zinc-800 bg-zinc-900 p-6 sm:rounded-2xl">
            ...
            <p className="mt-3 text-center font-mono text-[9px] leading-relaxed text-zinc-600">
              Broadcasts a real transfer via your connected TON wallet. Set your address in
              src/config.js before going live.
            </p>
          </div>
        </div>
      )}
...
/* ---------------- markdown report ---------------- */
function compileMarkdown(r, budget, alloc, tier) {
...
${line}
Generated by LEXCONV // GTM Terminal
`;
}
