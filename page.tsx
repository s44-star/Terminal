...
import { logLead, logPayment } from '@/lib/supabase';

/* ------------------------------------------------------------------ */
/*  Payment config — TON                                              */
/* ------------------------------------------------------------------ */

// Wallet address + TON price live in src/config.ts — the ONE file you edit.
const RECIPIENT_ADDRESS = TONKEEPER_WALLET_ADDRESS;

// While the address is still the placeholder, confirmPay() hard-blocks the
// transaction so no funds can ever be sent by mistake.
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (i < STAGES.length) {
        ...
        setTab('narrative');
        setPhase('complete');
        // Fire-and-forget lead capture. No-op if Supabase isn't configured.
        logLead({
  ...
  const confirmPay = async () => {
    ...
    try {
      ...
      // Fire-and-forget payment log. No-op if Supabase isn't configured.
      logPayment({
